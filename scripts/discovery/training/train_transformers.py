...
from april.fs import MODEL_DIR, get_event_log_files
...
def fit_and_save(use_event_attributes, dataset_name, ad, ad_kwargs=None, fit_kwargs=None):
    ...
    ad.save(str(MODEL_DIR / file_name))
...
def main():
    datasets = sorted([e.name for e in get_event_log_files() if e.name == 'paper-0.3-1'])

    number_encoding_layers = 6
    number_heads = 6
    max_number_epochs = 150
    ...
    for dataset in datasets:
            datasets_models.append(
                (dataset, dict(ad=TransformerModel, ad_kwargs=dict(num_enc_layers=number_encoding_layers, num_heads=number_heads),
                 fit_kwargs=dict(epochs=max_number_epochs, batch_size=50, validation_split=0.1, smoothing_extend=0.0))))
